...
# Bot package root = blackfox-agent-bot/
INSTALL_ROOT = Path(__file__).resolve().parents[2]

if INSTALL_ROOT in {
    Path("/opt/smart-support-bot"),
    Path("/opt/blackfox-agent-bot"),
}:
    # Prefer Smart Support Bot layout; keep legacy path compatible.
    preferred = Path("/opt/smart-support-bot-safety")
    legacy = Path("/opt/blackfox-bot-safety")
    SAFETY_ROOT = preferred if preferred.exists() or INSTALL_ROOT == Path("/opt/smart-support-bot") else legacy
else:
...
# Override via SAFETY_CONFIRM_CHAT_ID / BOT_ADMIN_IDS in .env
